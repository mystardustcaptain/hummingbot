...
class VWAPConfig(BaseClientModel):
    ...
    order_delay_time: int = Field(10, json_schema_extra={
        ...
        "prompt_on_new": True})
    price_activation_limit: Decimal = Field(Decimal("NaN"), json_schema_extra={
        "prompt": lambda mi: "Define an upper or lower bound for the bot to activate",
        "prompt_on_new": True})
...
class VWAPExample(ScriptStrategyBase):
    ...
    def __init__(self, connectors: Dict[str, ConnectorBase], config: VWAPConfig):
        ...
        self.vwap: Dict = {"connector_name": self.config.connector_name,
                           ...
                           "price_activation_limit": self.config.price_activation_limit}
    ...
    def on_tick(self):
        ...
        if self.last_ordered_ts < (self.current_timestamp - self.vwap["order_delay_time"]):
            ...
            elif self.vwap.get("status") == "ACTIVE" and self.price_activation_check():
    ...
    def price_activation_check(self):
        connector = self.vwap["connector"]
        is_buy = self.vwap["is_buy"]
        trading_pair = self.vwap["trading_pair"]
        price_activation_limit = self.vwap["price_activation_limit"]

        current_mid_price = connector.get_mid_price(trading_pair)

        if (is_buy and current_mid_price <= price_activation_limit) or (not is_buy and current_mid_price >= price_activation_limit):
            return True
        else:
            self.logger().info(f"Current mid price {current_mid_price} has not reached the activation limit {price_activation_limit}.")
            return False
